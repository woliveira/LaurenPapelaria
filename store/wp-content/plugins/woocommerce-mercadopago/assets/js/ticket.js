  ...
  $(function () {
    ...
    function validateInputs() {
      ...
      for (var j = 0; j < form_inputs.length; j++) {
        ...
        if (
          elementFocus.parentNode.style.display !== "none" &&
          (elementFocus.value === -1 || elementFocus.value === "")
        ) {
          elementFocus.focus();
          return false;
        }
    ...
    function validateDocTypeMLB(docnumber) {
      var mercado_pago_docnumber = document
        .getElementById("mp_cpf_cnpj_label")
        .innerText.includes("CPF")
        ? "CPF"
        : "CNPJ";
    ...
    function discountCampaignsHandler() {
      ...
      if (document.querySelector("#couponCodeTicket").value === "") {
        ...
        document.querySelector("#mpCouponErrorTicket").innerHTML =
          wc_mercadopago_ticket_params.coupon_empty;
        document.querySelector("#couponCodeTicket").style.background = null;
        document.querySelector("#applyCouponTicket").value =
          wc_mercadopago_ticket_params.apply;
        ...
        document.querySelector("#couponCodeTicket").style.background =
          "url(" +
          wc_mercadopago_ticket_params.loading +
          ") 98% 50% no-repeat #fff";
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.status === 200) {
            ...
            document.querySelector("#mpCouponApplyedTicket").style.display =
              "block";
            document.querySelector("#discountTicket").value =
            ...
            document.querySelector("#mpCouponApplyedTicket").innerHTML =
              ...
              "</strong> " +
              wc_mercadopago_ticket_params.discount_info2 +
              " " +
              response.response.name +
              ".<br>" +
              wc_mercadopago_ticket_params.discount_info3 +
              " <strong>" +
              currencyIdToCurrency(response.response.currency_id) +
              " " +
              Math.round(getAmountWithoutDiscount() * 100) / 100 +
              "</strong><br>" +
              wc_mercadopago_ticket_params.discount_info4 +
              " <strong>" +
              currencyIdToCurrency(response.response.currency_id) +
              " " +
            ...
            document.querySelector("#couponCodeTicket").style.background =
              "url(" +
              wc_mercadopago_ticket_params.check +
              ") 94% 50% no-repeat #fff";
            document.querySelector("#couponCodeTicket").style.border =
              "1px solid #cecece";
            document.querySelector("#applyCouponTicket").value =
              wc_mercadopago_ticket_params.remove;
            document.querySelector("#campaign_idTicket").value =
              response.response.id;
            document.querySelector("#campaignTicket").value =
              response.response.name;
          } else {
            coupon_of_discounts.status = false;
            document.querySelector("#mpCouponApplyedTicket").style.display =
            ...
            document.querySelector("#couponCodeTicket").style.background =
              "url(" +
              wc_mercadopago_ticket_params.error +
              ") 94% 50% no-repeat #fff";
            document.querySelector("#applyCouponTicket").value =
              wc_mercadopago_ticket_params.apply;
            document.querySelector("#discountTicket").value = 0;
    ...
    function getAmount() {
      return (
        document.getElementById("amountTicket").value -
        document.getElementById("discountTicket").value
      );
    }
  });
})(jQuery);
